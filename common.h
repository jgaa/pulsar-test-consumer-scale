...
struct Config {
    ...
    bool producer = true;
    size_t consumers = 1;
    size_t consumers_per_client = 512;
...
public:
    ...
    Base(boost::asio::io_context& ctx, const Config& conf)
        : ctx_{ctx}, conf_{conf}
    {
        pulsar::ClientConfiguration cc;
        cc.setIOThreads(conf_.pulsar_client_threads);
        cc.setUseTls(false);
        cc.setStatsIntervalInSeconds(conf_.stats_interval);
        cc.setLogger({});
        client_.emplace(conf_.pulsar_url, cc);
    }
    ...
    static std::unique_ptr<Base> CreateProducer(boost::asio::io_context& ctx, const Config& conf);
    static std::unique_ptr<Base> CreateConsumer(
 ...
 protected:
    ...
    std::optional<pulsar::Client> client_;
