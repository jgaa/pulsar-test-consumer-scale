...
{
    struct Generator {
        Generator(Sender& sender) : sender_{sender}, strand_{sender_.ctx_} {}
    ...
    private:
        ...
        void Feed(size_t burstCount) {
            ...
            producer_.sendAsync(msg, [this, current_id, &queue_full_flag](pulsar::Result res, const pulsar::MessageId& messageId) mutable {
...
public:
    Sender(boost::asio::io_context& ctx, const Config& conf)
        : Base(ctx, conf)
    {
    }
    ...
    void OnDone(Generator& /*generator*/) {
        ctx_.post([this] () {
            ...
            result_.ok_messages = generator_->GetNumMessagesSent();
            result_.failed_messages = generator_->GetNumMessagesFailed();
            result_.aggregated_avg_per_sec = generator_->GetAvgMessagePerSecond();
            result_.avg_per_sec = result_.aggregated_avg_per_sec ;
...
protected:
    ...
    generator_ = make_shared<Generator>(*this);

    client_->createProducerAsync(topic_str, pc,
                           [this, topic_str] (pulsar::Result res, pulsar::Producer producer) {

       if (res != pulsar::ResultOk) {
           BOOST_LOG_TRIVIAL ( error ) << generator_->Name() << ": Failed to create producer for " << topic_str;
           BOOST_LOG_TRIVIAL ( error ) << "Calling Shutdwon()";
           Shutdown();
           return;
       }

       generator_->SetProducer(move(producer));
       BOOST_LOG_TRIVIAL ( debug ) << generator_->Name()
                                   << " ready to produce to "
                                   << topic_str;
...
std::unique_ptr<Base> Base::CreateProducer(boost::asio::io_context& ctx, const Config& conf) {
    return make_unique<Sender>(ctx, conf);
}
